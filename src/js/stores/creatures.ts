...
export type Attribute
    ...
    | Readonly<{ type: 'counter', value: CounterID }>;
...
export function create(creature: Creature): CREATURE_ACTIONS {
    return { type: '$CREATURE/CREATE', creature };
}

export function addCondition(creature: Creature | CreatureID, condition: Condition): CREATURE_ACTIONS {
...
export function removeCondition(creature: Creature | CreatureID, condition: Condition): CREATURE_ACTIONS {
    const creatureId = creature instanceof Creature ? creature.id : creature;
    return { type: '$CREATURE/ADD_CONDITION', creatureId, condition };
}
...
function mapReducer(state: CreatureDict['map'], action: CREATURE_ACTIONS) {
    switch (action.type) {
...
function idsReducer(state: CreatureDict['ids'], action: CREATURE_ACTIONS) {
    switch (action.type) {
        case '$CREATURE/CREATE':
...
export function creatureReducer(state: CreatureDict, action: CREATURE_ACTIONS) {
